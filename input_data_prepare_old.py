...
class input_data_one_day_one_stock:
    ...
    def normalized_input_data(self):
        ...
        input_8 = np.hstack((data[1:,[3,4]], diff_data, ask1_bid1_delta, data[1:,[27,28,29]]))  # 除了ofi以外的其他数据,加上ytrue 
    ...
    def triple_barrier_labeling(self, bar = 'time'):
        ...
        for t in second_time[1:]:              # v1
        # for t in second_time[1:][::self.part_fraction]:              # v2
        ...
        # 从第二个return_data开始计算是否触碰边界,即第三个midprice时间开始计算label    v1
        up_list = list((1 + n * volitility_array[1:]) * data[2:,26])          # 价格上界
        down_list = list((1 - n * volitility_array[1:]) * data[2:,26])        # 价格下界
        ...
        # label_list = []
        # # for n, t in enumerate(second_time[2:]):
        # for t in data[2::self.part_fraction, 25]:
        #     if bar == 'time':
...
class input_data_one_stock_all_days():
    ...
    def train_n_test_data(self, n_threds = 32, ft_num=15, tmp_x =3, part_fraction=1):   # 默认用32个核跑
        '''
        ...
        scaler = StandardScaler().fit(np.concatenate(train_data))         # 得到训练集的标准化参数

        for day in range(len(train_data)):
            if len(train_data[day]) > 0:     # v2
                train_day = scaler.transform(train_data[day]) # 分出每天的train_data，避免跨天数据重叠
            for second in range(len(train_day)-timesteps):     # 每天按照时间分出sample   
                x_train_list.append(train_day[second:second + timesteps])

        x_train = np.concatenate(x_train_list, axis=0).reshape(-1, timesteps, ft_num)[:,:,:-1]
        y_train_sequential = np.concatenate(x_train_list, axis=0).reshape(-1, timesteps, ft_num)[:,:,-1]      # ytrain序列,shape = (247685,timesteps, 3)
        ...
        for day in range(len(test_data)):
            test_day = scaler.transform(test_data[day]) # 分出每天的test_data，避免跨天数据重叠,使用测试集的数据进行标准化
            for second in range(len(test_day)-timesteps):     # 每天按照时间分出sample   
                x_test_list.append(test_day[second:second + timesteps])

        x_test = np.concatenate(x_test_list, axis=0).reshape(-1, timesteps, ft_num)[:,:,:-1]
        y_test_sequential = np.concatenate(x_test_list, axis=0).reshape(-1, timesteps, ft_num)[:,:,-1]      # ytest序列,shape = (247685,timesteps, 3)
